...
class ImageChopper(BaseEstimator, TransformerMixin):
    ...
    def fit(self,file_location,nxpixels=config.DEFAULT_NXPIX,nypixels=config.DEFAULT_NYPIX) -> 'ImageChopper':
        ...
        self.even_grid = np.full([np.shape(dataset)[0],self.new_height,self.new_width],np.nan)

        self.even_grid[:,self.pad_b:self.new_height-self.pad_t,self.pad_l:self.new_width-self.pad_r] = dataset
    ...
    def transform(self, X: pd.DataFrame = None) -> gpd.GeoDataFrame:
        ...
        for i in range(nwidth//self.npixwidth):
            ...
            for j in range(nheight//self.npixheight):
                ...
                #make the slice
                slice_ij = self.even_grid[:,inc_height:inc_height+self.npixheight,inc_width:inc_width+self.npixwidth]
                ...
                bounds = (\
                          ...
                          (tl_w+self.lon_per_pixel*self.npixwidth,tl_h-self.lat_per_pixel*self.npixheight),\
